...
# Save age-eligible dataset with missing values retained
EDData_MI <- EDData
...
# -----------------------------------------------------------------------------
# Save dataset for multiple imputation sensitivity analysis
# -----------------------------------------------------------------------------

output_file_mi <- file.path(output_dir, "ED_fbsv_rses_ptq_MI.rds")

saveRDS(EDData_MI, output_file_mi)

cat("Saved MI dataset to:", output_file_mi, "\n")